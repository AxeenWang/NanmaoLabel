...
- [x] T016 執行 `powershell.exe -Command "cd '$(wslpath -w .)'; dotnet build NanmaoLabelPOC"` 確認編譯成功且無警告
  - ✅ 編譯成功：0 個警告、0 個錯誤

- [x] T017 執行手動測試案例 AC-01：單擊 ListView 任一項目，確認預覽區顯示標籤
  - ✅ PASS

- [x] T018 執行手動測試案例 AC-02：確認標籤預覽包含所有元素（文字、條碼、QR Code）
  - ✅ PASS

- [x] T019 執行手動測試案例 AC-03：切換標籤格式（QW075551-1 ↔ QW075551-2），確認預覽區即時更新
  - ✅ PASS

- [x] T020 執行手動測試案例 AC-04：切換不同 ListView 項目，確認預覽區正確更新
  - ✅ PASS

- [x] T021 執行手動測試案例 AC-05：確認無選取項目時顯示空白狀態提示
  - ✅ PASS

- [x] T022 執行手動測試案例 AC-06：輸出 PDF，對照預覽區與 PDF 版面一致性（誤差 ≤ ±0.5mm）
  - ✅ PASS

- [x] T023 程式碼審查：確認無魔術數字、無空 catch 區塊、遵循命名規範
  - ✅ 無魔術數字：所有常數定義於 #region Constants
  - ✅ 遵循命名規範：PascalCase 方法名、_camelCase 私有欄位
  - ✅ XML 文件註解完整，含 [ref:] 追溯標記
  - ✅ 空 catch 區塊已修正：第 481-497 行新增 Debug.WriteLine() 記錄錯誤（符合憲章「必須記錄或重新拋出」）
